<template>
  <div>
    ...
    <el-container>
      <el-header>
        <div class="menu">
          <li :class="curIndex === index?'cur':''" v-for="(item, index) in permission_routes" :key="index" @click="handleLoadChild(item, index)">{{item.meta.title}}</li>
        </div>
      </el-header>
      <el-container>
        <el-menu
          ...
          mode="vertical"
        >
          <sidebar-item v-for="route in parrentRoutes" :key="route.path" :item="route" :base-path="route.path"></sidebar-item>
        </el-menu>
