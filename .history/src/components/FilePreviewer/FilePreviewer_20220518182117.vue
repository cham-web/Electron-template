...
export default {
  ...
  methods: {
    async initBufferFile (buffer, propFile) {
      this.loading = true
      try {
        this.last = await this.displayResult(buffer, propFile)
        this.loading = false
      } catch (e) {
        console.error(e)
      } finally {
    ...
    async initFile (propFile) {
      this.loading = true
      try {
        const arrayBuffer = await readBuffer(propFile)
        this.loading = false
        this.last = await this.displayResult(arrayBuffer, propFile)
      } catch (e) {
        console.error(e)
      } finally {
        this.loading = false
    ...
    async handleChange (e) { // input选择文件
      this.loading = true
      try {
